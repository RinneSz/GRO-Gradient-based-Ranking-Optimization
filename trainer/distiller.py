...
class NoDataRankDistillationTrainer(metaclass=ABCMeta):
    def __init__(self, args, model_code, model, bb_model, surrogate_test_loader, bb_test_loader, export_root, margin_topk=0.5, margin_neg=0.5):
        self.args = args
        ...
        self.defense_mechanism = args.defense_mechanism
        ...
        self.surrogate_test_loader = surrogate_test_loader
        self.bb_test_loader = bb_test_loader
    ...
    def calculate_metrics(self, batch, val=True, similarity=False):
        self.model.eval()
        self.bb_model.eval()
    ...
    def generate_autoregressive_data(self, batch_size=50):
        k=self.args.k
            ...
            with torch.no_grad():
                if isinstance(self.bb_model, BERT):
                    ...
                    for j in range(self.max_len - 1):
                        ...
                        _, sorted_items = defense(labels[:, 1:-1], k, mechanism=self.defense_mechanism)
                    ...
                    for j in range(self.max_len - 1):
                        ...
                        _, sorted_items = defense(labels[:, 1:], k, mechanism=self.defense_mechanism)
    ...
    def validate(self, val_loader, epoch, accum_iter):
        ...
        torch.save(self.model.state_dict(), os.path.join(self.export_root, 'models', 'last_model.pth'))
    ...
    def test(self):
        try:
            ...
            wb_model = torch.load(os.path.join(self.export_root, 'models', 'last_model.pth'))
            self.model.load_state_dict(wb_model)
        ...
        average_meter_set = AverageMeterSet()
        with torch.no_grad():
            tqdm_dataloader = tqdm(self.surrogate_test_loader)
            for batch_idx, batch in enumerate(tqdm_dataloader):
                metrics = self.calculate_metrics(batch, val=False, similarity=True)
                self._update_meter_set(average_meter_set, metrics)
                self._update_dataloader_metrics(
                    tqdm_dataloader, average_meter_set)

            average_metrics = average_meter_set.averages()
            average_metrics.update(name='name', value='surrogate_model_test')
    ...
    def bb_model_test(self):
        ...
        average_meter_set = AverageMeterSet()
        with torch.no_grad():
            tqdm_dataloader = tqdm(self.bb_test_loader)
            for batch_idx, batch in enumerate(tqdm_dataloader):
                if isinstance(self.bb_model, BERT):
                    seqs, candidates, labels = batch
                    seqs, candidates, labels = seqs.to(self.device), candidates.to(self.device), labels.to(self.device)
                    ...
                    if not self.args.rankall:
                        metrics = recalls_and_ndcgs_for_ks_defensed(scores.gather(1, candidates), labels, self.metric_ks, mechanism=self.defense_mechanism)
                    else:
                        metrics = recalls_and_ndcgs_for_ks_defensed_rankall(scores[:, 1:-1], candidates, labels, self.metric_ks,
                                                                    mechanism=self.defense_mechanism)
                elif isinstance(self.bb_model, SASRec):
                    seqs, candidates, labels = batch
                    seqs, candidates, labels = seqs.to(self.device), candidates.to(self.device), labels.to(self.device)
                    scores = self.bb_model(seqs)[:, -1, :]
                    if not self.args.rankall:
                        ...
                        metrics = recalls_and_ndcgs_for_ks_defensed_rankall(scores[:, 1:], candidates, labels, self.metric_ks,
                                                                    mechanism=self.defense_mechanism)
                elif isinstance(self.bb_model, NARM):
                    seqs, lengths, candidates, labels = batch
                    seqs, candidates, labels = seqs.to(self.device), candidates.to(self.device), labels.to(self.device)
                    ...
                    if not self.args.rankall:
                        metrics = recalls_and_ndcgs_for_ks_defensed(scores.gather(1, candidates), labels,
                                                                    self.metric_ks, mechanism=self.defense_mechanism)
                    else:
                        metrics = recalls_and_ndcgs_for_ks_defensed_rankall(scores[:, 1:], candidates, labels,
                                                                            self.metric_ks,
                                                                            mechanism=self.defense_mechanism)
