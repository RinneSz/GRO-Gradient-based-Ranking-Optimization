...
def distill(args, bb_model_root=None, export_root=None, resume=False):
    ...
    if bb_model_root == None:
        bb_model_root = 'experiments/gro' + bb_model_code + '2' + bb_model_code + '/' + args.dataset_code + '/' + str(args.lamb)
    if export_root == None:
        folder_name = bb_model_code + '2' + args.model_code + '_autoregressive' + str(args.num_generated_seqs) + 'gro'
        export_root = 'experiments/distillation_rank_gro/' + folder_name + '/' + args.dataset_code + '/' + str(args.lamb)


    bb_model.load_state_dict(torch.load(os.path.join(bb_model_root, 'models', 'last_target_model.pth'), map_location='cpu'))
    if resume:
        try:
